...
ANNOTATION_SYSTEM_PROMPT = (
    ...
    "Simplified Chinese. Also return 1–3 focus_points per paragraph. Each focus point must copy "
    "an exact important source sentence or short phrase in quote, label it with kind, and give a "
    "separate beginner-facing explanation that teaches that exact wording rather than repeats the "
    "paragraph summary. Use kind=formula for every formula that needs teaching, provide its "
    "formula_latex, explain symbols and change direction, and include one very small numerical "
    "example. Preserve source anchors and source quotes exactly. When translation is requested, "
...
def build_annotation_prompt(
    ...
    return (
        ...
        "- focus_points: choose 1–3 genuinely important, exact quotations from this same source paragraph. "
        "Do not paraphrase quote and do not quote a whole long paragraph. For each point provide kind "
        "(claim, term, mechanism, evidence, formula, or limitation) and a separate, detailed Chinese "
        "explanation of that exact phrase or sentence. These cards are what a reader sees after clicking "
        "a highlighted part of the original text, so teach the concept and reasoning rather than restating "
        "the paragraph summary. If the source contains a formula, add a kind=formula focus point, supply "
        "formula_latex, explain every important symbol and direction of change, and include a tiny numerical example.\n"
